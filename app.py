...
if st.session_state.get('active_tab') == t('btn_bimaks_tech') and not st.session_state.get('show_settings'):
    ...
    # F. BAYİ SDS/TDS ÜRETİCİ (V 126.0 - E-mail ve Çift Nokta Fix)
        ...
        with c_p1:
            ...
            if doc_type == "SDS":
                # V 126.0 - Çift Nokta Korumalı Temiz Sözlük
                with st.expander("🧠 Akıllı Belge Düzenleyici (SDS Otonom)", expanded=True):
                    ...
                    # Motora gönderilecek Lazer Kesim Komutları (Arama anahtarları saf haliyle eklendi, ayraçlar bizim kontrolümüzde)
                    auto_data = {
                        "ÜRÜN ADI": ("", new_prod),
