...
if st.session_state.get('active_tab') == t('btn_bimaks_tech') and not st.session_state.get('show_settings'):
    ...
    # E. TEKLİF OLUŞTUR
        ...
        with st.expander(_("📄 Antetli Kağıt Ayarı", "📄 Letterhead Setup", "📄 Настройка бланка", "📄 إعداد الترويسة", "📄 Config. En-tête", "📄 Config. Membrete"), expanded=True):
            ...
            ut = st.file_uploader(_("Yeni Antet Yükle:", "Upload New Letterhead:", "Загрузить yeni бланк:", "تحميل ترويسة جديدة:", "Nouveau En-tête :", "Nuevo Membrete:"), type=['png', 'jpg', 'jpeg', 'pdf'], key="quote_antet")
    ...
    # G. SIFIRDAN SDS/TDS ÜRETİCİ (V 132.0 - KATI ŞABLON ENTEGRASYONU)
        ...
        if st.button("AI ile Belgeyi PDF Olarak Üret", type="primary"):
            if not gen_prod_name or not gen_ingredients:
                ...
                with st.spinner("AI kimyasal formülü analiz ediyor ve uluslararası normlara göre PDF üretiyor... (Bu işlem 15-30 saniye sürebilir)"):
                    res_text = logic.generate_sds_from_recipe_with_gemini(
                        gen_prod_name, 
                        gen_prod_type, 
                        gen_ingredients, 
                        doc_choice, 
