...
# --- YENİ MİMARİ MOTORLARININ İÇERİ AKTARILMASI ---
import db_engine
import math_engine
        ...
        with tab1:
            ...
            if st.button("Giriş", type="primary", use_container_width=True):
                with st.spinner("Doğrulanıyor..."):
                    success, data = db_engine.login_user(log_user, log_pass)
                    if success:
            ...
            if st.button("Kayıt Ol", use_container_width=True):
                if len(reg_user) < 3 or len(reg_pass) < 3:
                    ...
                    with st.spinner("Kayıt yapılıyor..."):
                        success, msg = db_engine.register_user(reg_user, reg_pass, "Yeni Üye")
